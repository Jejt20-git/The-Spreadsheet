    ...
    private double evaluate(Expression e, Spreadsheet sheet, Set<Coordinate> visiting) {
        ...
        if (e instanceof FunctionCall f) {
            ...
            if (name.equals("SUM") || name.equals("AVERAGE")) {
                ...
                if (args.size() != 1 || !(args.get(0) instanceof RangeRef range)) {
                    throw new IllegalArgumentException(name + "() expects a range, e.g. " + name + "(A1;A10)");
                }

                double sum = 0.0;
                int count = 0;

                for (Coordinate coord : range.expand()) {
                    Content content = sheet.getCellContent(coord);

                    // Decide rule: treat empty cells as 0 for SUM, and ignore in AVERAGE
                    if (content == null) {
                        if (name.equals("SUM")) {
                            // sum += 0;
                        }
                        // for AVERAGE: ignore empties
                        continue;
                    }

                    // Evaluate each cell to a number
                    double v = evaluateContentAsNumber(content, sheet, visiting);
                    sum += v;
                    count++;
                }

                if (name.equals("AVERAGE")) {
                    if (count == 0) throw new IllegalArgumentException("AVERAGE() over empty range");
                    return sum / count;
                }
            ...
            // Existing MIN/MAX behaviour (many args)
            if (args.isEmpty()) {
